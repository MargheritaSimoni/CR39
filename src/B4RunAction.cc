...
: G4UserRunAction()
{
    ...
    analysisManager->CreateH1("SecTrackL","track lengths of secondaries ",200, -0.5*mm, 0.5*mm,"mm"); // third entry is type of binning (log binning)//nb min can't be 0 in log scale//nb da anche il max dell'istogramma
    analysisManager->SetH1XAxisTitle(5, "z [mm]");
...
*/
    ...
    analysisManager->CreateH2("AvsTrackLength","Atomic number vs tracklength", 200, 0.*um, 30*um, 21, -0.5, 20.5, "um");
    analysisManager->SetH2XAxisTitle(3,  "Track length [um]");
    analysisManager->SetH2YAxisTitle(3,"Atomic mass");

    analysisManager->CreateH2("EvsTrackLength","Energy vs tracklength", 200, 0*MeV, 1.1*MeV, 200, 0.*um, 30*um, "MeV", "um");
    analysisManager->SetH2XAxisTitle(4,"Energy [MeV]" );
    analysisManager->SetH2YAxisTitle(4,"Track length [um]");

    analysisManager->CreateH2("AvsE","Atomic number vs energy", 200, 0*MeV, 1.1*MeV, 21, -0.5, 20.5, "MeV");
